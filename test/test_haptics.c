...
void test_Haptics_pause_all(){
	...
	HapticsPlayers[0].device = &haptic1;
	Haptics_pause_all();
	TEST_ASSERT_EQUAL_INT(1, _SDL_HapticPause_called);
	HapticsPlayers[0].device = NULL;
}
...
void test_Haptics_set_enabled(){
	...
	TEST_ASSERT_EQUAL_INT(1, Haptics_enabled);

	Haptics_set_enabled(0);
	TEST_ASSERT_EQUAL_INT(0, Haptics_enabled);
}

void test_Haptics_player_set_enabled(){
	Haptics_player_set_enabled(0, 1);
	TEST_ASSERT_EQUAL_INT(1, HapticsPlayers[0].enabled);
	
	Haptics_player_set_enabled(0, 0);
	TEST_ASSERT_EQUAL_INT(0, HapticsPlayers[0].enabled);
}
...
void test_Haptics_open_joystick_for_player(){
	...
	TEST_ASSERT_NOT_NULL_MESSAGE(HapticsPlayers[0].device, "Player device should not be NULL.");
...
void test_Haptics_register_effect(){
	...
	HapticsEffectDefinitions[0] = effect1;

	SDL_HapticEffect effect2 = { .type = SDL_HAPTIC_TRIANGLE };
	SDL_Haptic device2 = {};
	HapticsPlayers[0].device = &device2;

	TEST_ASSERT_EQUAL_INT(1, Haptics_register_effect(&effect2));
	TEST_ASSERT_EQUAL_INT(SDL_HAPTIC_TRIANGLE, HapticsEffectDefinitions[1].type);
	TEST_ASSERT_EQUAL_INT_MESSAGE(1, _SDL_HapticNewEffect_called, "Effect should be added to the device.");
}

void test_Haptics_register_effect_at(){
	SDL_HapticEffect effect1 = { .type = SDL_HAPTIC_SINE };
	HapticsEffectDefinitions[1] = effect1;
	...
	TEST_ASSERT_EQUAL_INT_MESSAGE(SDL_HAPTIC_TRIANGLE, HapticsEffectDefinitions[1].type, "Effect definition 1 should have been overwritten.");
	TEST_ASSERT_EQUAL_INT_MESSAGE(1, _SDL_HapticNewEffect_called, "Effect should be added to the device.");
}
...
void test_Haptics_remove_effect(){
	...
	HapticsEffectDefinitions[0] = effect1;
	SDL_Haptic device1 = {};
	HapticsPlayers[0].device = &device1;
	HapticsPlayers[0].effect[0] = 1;

	Haptics_remove_effect(0);
	TEST_ASSERT_EQUAL_INT(0, HapticsEffectDefinitions[0].type);
	TEST_ASSERT_EQUAL_INT_MESSAGE(1, _SDL_HapticDestroyEffect_called, "Effect should be removed from device.");
	TEST_ASSERT_EQUAL_INT_MESSAGE(-1, HapticsPlayers[0].effect[0], "Effect should be removed from the player.");
}
...
void test_Haptics_set_effect(){
	...
	TEST_ASSERT_EQUAL_INT(SDL_HAPTIC_TRIANGLE, HapticsEffectDefinitions[0].type);
...
void test_Haptics_player_run_effect(){
	...
	HapticsPlayers[0].enabled = 0;
	Haptics_enabled = 1;
	Haptics_player_run_effect(0, 0, 1);
	TEST_ASSERT_EQUAL_INT(0, _SDL_HapticRunEffect_called);

	// player enabled flag is honored
	HapticsPlayers[0].enabled = 1;
	Haptics_player_run_effect(0, 0, 1);
